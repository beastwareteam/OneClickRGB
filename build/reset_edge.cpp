...
int main(int argc, char* argv[]) {
    ...
    // Step 1: Reset to mode 0x06 (wie es vorher war)
    EVisionQuery(dev, 0x01, 0, nullptr, 0, nullptr);
    Sleep(20);
    
    // Exakt wie damals VOR der Änderung: 06 04 00 00 00 00 22 ff 00 00
    uint8_t reset[10] = {0x06, 0x04, 0x00, 0x00, 0x00, 0x00, 0x22, 0xff, 0x00, 0x00};
    EVisionQuery(dev, 0x06, edge_offset, reset, 10, nullptr);
    std::cout << "Reset to original state (mode 0x06)" << std::endl;
    
    EVisionQuery(dev, 0x02, 0, nullptr, 0, nullptr);
    Sleep(1000);
    
    // Step 2: Jetzt auf mode 0x04 setzen (wie es funktioniert hat)
    EVisionQuery(dev, 0x01, 0, nullptr, 0, nullptr);
    Sleep(20);
    
    // Exakt wie damals NACH der Änderung: 04 04 00 00 00 00 22 ff 00 01
    uint8_t newcfg[10] = {0x04, 0x04, 0x00, 0x00, 0x00, r, g, b, 0x00, 0x01};
    EVisionQuery(dev, 0x06, edge_offset, newcfg, 10, nullptr);
    
    std::cout << "Set to Static mode (0x04) with RGB(" << (int)r << "," << (int)g << "," << (int)b << ")" << std::endl;
