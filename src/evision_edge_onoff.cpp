// EVision Edge LED Control - Toggle ON/OFF to trigger update
#include <windows.h>
#include <iostream>
#include <iomanip>
#include <hidapi.h>
...
int main(int argc, char* argv[]) {
    ...
    // STEP 1: Turn OFF (Byte[9] = 0x00) with mode 0x06
    EVisionQuery(dev, 0x01, 0, nullptr, 0, nullptr);
    Sleep(20);

    uint8_t off_config[10] = {0x06, 0x04, 0x00, 0x00, 0x00, r, g, b, 0x00, 0x00};  // OFF
    EVisionQuery(dev, 0x06, edge_offset, off_config, 10, nullptr);
    std::cout << "Step 1: Set OFF (mode 0x06, byte[9]=0x00)" << std::endl;

    EVisionQuery(dev, 0x02, 0, nullptr, 0, nullptr);
    Sleep(200);

    // STEP 2: Turn ON with mode 0x04 (Static) and color
    EVisionQuery(dev, 0x01, 0, nullptr, 0, nullptr);
    Sleep(20);

    uint8_t on_config[10] = {0x04, 0x04, 0x00, 0x00, 0x00, r, g, b, 0x00, 0x01};  // ON
    EVisionQuery(dev, 0x06, edge_offset, on_config, 10, nullptr);
    std::cout << "Step 2: Set ON (mode 0x04, byte[9]=0x01) with color" << std::endl;
