...
int main(int argc, char* argv[]) {
    ...
    // Step 1: Set to Rainbow mode first (force mode change)
    uint8_t rainbow[10] = {0x01, 0x04, 0x00, 0x00, 0x00, r, g, b, 0x00, 0x01};
    EVisionQuery(dev, 0x06, edge_offset, rainbow, 10, nullptr);
    std::cout << "Set Rainbow mode" << std::endl;

    EVisionQuery(dev, 0x02, 0, nullptr, 0, nullptr);
    Sleep(500);

    // Step 2: Set to Static mode with color
    EVisionQuery(dev, 0x01, 0, nullptr, 0, nullptr);
    Sleep(20);

    uint8_t stat[10] = {0x04, 0x04, 0x00, 0x00, 0x00, r, g, b, 0x00, 0x01};
    EVisionQuery(dev, 0x06, edge_offset, stat, 10, nullptr);
    std::cout << "Set Static RGB(" << (int)r << "," << (int)g << "," << (int)b << ")" << std::endl;
